...
export default function QuestionsPanel({
  ...
  async function handlePick(libraryQuestionId: string, questionText: string) {
    await addQuestion({ libraryQuestionId, questionText, sessionId, meetingId, projectId });
    router.refresh();
  }

  async function handleAddCustom() {
    if (!customQuestion.trim()) return;
    await addQuestion({ libraryQuestionId: null, questionText: customQuestion.trim(), sessionId, meetingId, projectId });
    setCustomQuestion("");
    router.refresh();
  }
  ...
  return (
    <div className="space-y-6">
      <div className="space-y-4">
        {questions.length === 0 && (
          <p className="text-sm text-gray-700">No questions added yet — pick some below.</p>
        )}
        {questions.map((q) => (
          <div key={q.id} className="border rounded-lg p-4">
            <div className="flex items-start justify-between gap-2 mb-2">
              ...
              <button
                onClick={async () => {
                  await removeQuestion(q.id, { sessionId, meetingId, projectId });
                  router.refresh();
                }}
            ...
            <AnswerBox
              ...
              paths={{ sessionId, meetingId, projectId }}
            />
          </div>
        ))}
      </div>
          ...
          <div className="space-y-3">
            ...
            <div className="border rounded-lg divide-y max-h-80 overflow-y-auto">
              {Object.keys(grouped).length === 0 && (
                <p className="p-3 text-sm text-gray-700">No more library questions to add.</p>
              )}
              {Object.entries(grouped).map(([category, qs]) => (
                <div key={category}>
                  <p className="px-3 pt-2 text-xs font-semibold text-gray-700 uppercase">{category}</p>
                  {qs.map((q) => (
                    <button
                      key={q.id}
            ...
            <button onClick={() => setPicking(false)} className="text-sm text-gray-600 hover:text-gray-900">
              Done adding
            </button>
          </div>
        )}
...
function AnswerBox({
  ...
  return (
    <div>
      ...
      />
      {saving && <p className="text-xs text-gray-700 mt-1">Saving...</p>}
    </div>
  );
}