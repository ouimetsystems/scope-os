    ...
    <div className="p-6 max-w-2xl mx-auto space-y-6">
      <div>
        ...
        <div className="flex items-center justify-between mt-1">
          ...
          <span className="text-xs uppercase tracking-wide bg-gray-100 text-gray-800 px-2 py-1 rounded-full">
            {quote.status} · v{quote.version}
          </span>
        </div>
        <p className="text-sm text-gray-700">{quote.clients?.company_name}</p>
      </div>
      ...
      <div className="border rounded-lg p-4 bg-gray-50 flex gap-6 text-sm">
        <span>
          <span className="text-gray-700">One-time total:</span>{" "}
      ...
      <section>
        <h2 className="font-medium mb-2 text-sm text-gray-700 uppercase tracking-wide">
          Included Features
        </h2>
      ...
      {unpriced.length > 0 && (
        <section>
          ...
          <p className="text-xs text-gray-700 mt-1">
            Set a price on this feature on the Problems & Features tab, then generate a new quote version.
          </p>
        </section>
      )}
    </div>
